...
class DriveClient:
    ...
    def upload_or_update_file(
        ...
        media = MediaInMemoryUpload(local_file.read_bytes(), mimetype=mimetype, resumable=False)
        if file_id:
...
class TransactionsHandler(FileSystemEventHandler):
    ...
    def _sync_transaction(self, source_csv: Path, day_value: int) -> None:
        ...
        tmp_csv = Path(tempfile.gettempdir()) / f"{uuid.uuid4().hex}_{drive_csv_name}"
    ...
    def _rebuild_period_main(self, period_folder_id: str) -> None:
        ...
        output = Path(tempfile.gettempdir()) / f"{uuid.uuid4().hex}_main.xlsx"
    ...
    def _rebuild_global_main(self, root_id: str) -> None:
        ...
        output = Path(tempfile.gettempdir()) / f"{uuid.uuid4().hex}_main_total.xlsx"
